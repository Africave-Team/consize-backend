...
export const createSignature = catchAsync(async (req: Request, res: Response) => {
    ...
    const signatures = await signatureService.createSignature({ name: name, email: email, position })
    ...
    res.status(httpStatus.OK).send({data: signatures, message: "signature details created and notification mail sent to the client"})
})
